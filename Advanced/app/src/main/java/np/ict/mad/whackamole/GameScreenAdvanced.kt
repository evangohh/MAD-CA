...
import np.ict.mad.whackamole.advanced.db.ScoreEntity
...
fun GameScreenAdvanced(
    ...
    var moleIndex by remember { mutableIntStateOf(0) }
    ...
    suspend fun saveScoreToDbIfLoggedIn(finalScore: Int) {
    ...
    fun endGame() {
        isRunning = false
        showGameOver = true
        // Save score (DB insert) on game end
        // We launch via LaunchedEffect below to keep it lifecycle-safe
    }
    ...
    // Mole movement loop (700–1000ms)
    LaunchedEffect(isRunning, gameId) {
        if (!isRunning) return@LaunchedEffect

        while (isRunning && timeLeft > 0) {
            val interval = Random.nextLong(700, 1001)
            delay(interval)
            moleIndex = Random.nextInt(9)
        }
    }

    // Save score once when game over becomes true
    LaunchedEffect(showGameOver) {
        if (showGameOver) {
            saveScoreToDbIfLoggedIn(score)
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Wack-a-Mole (Advanced)") },
                actions = {
        ...
        Column(
            ...
            if (showGameOver) {
                ...
                Text("Saved to DB: ${currentUser?.username ?: "No user"}")
            }
        }
    }
}
